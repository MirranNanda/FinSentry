...
- **Scraper schedule** — active (`supabase/enable_daily_scrape.sql`). Once a day (09:00 UTC), `scrape-reels` checks every active influencer, saves at most 5 new Reels (newest first), and refreshes at most 3 already-seen Reels whose stored link had gone dead — deliberately conservative so 5+3=8/day fits inside Gemini's free-tier cap of 20 analyses/day, leaving headroom for the retry job. Each new or refreshed Reel triggers `analyze-video` automatically (Phase 6/7.1 triggers), so this is genuinely hands-off: it keeps running in Supabase's own infrastructure whether or not this machine, VS Code, or any Claude session is open. To pause: `select cron.unschedule('scrape-watchlist-daily');`. To change the pace, edit `max_new`/`max_refresh` in that SQL file and re-run it.
- **Dead-link self-healing** — Instagram's `video_url` is a short-lived signed CDN link (observed to die within ~24h); once it does, no amount of retrying the same URL will ever succeed. The only real fix is a fresh scrape of that same Reel, which hands back a live link — see the "Self-healing dead links" comment in `supabase/functions/scrape-reels/index.ts`. The retry job (below) knows this and skips permanently-dead rows so it doesn't waste its slots on them.
